...
cd ScreenshotMacro
just build                             # 또는 swift build
just test                              # 단위 테스트
just app-smoke                         # .app 번들/아이콘/서명 smoke
git config core.hooksPath .githooks    # (기여자) push마다 테스트 자동 실행
```

`just`가 없다면 Swift 명령과 스크립트를 직접 실행해도 됩니다:
...
설치 가능한 앱 번들은 아래 명령으로 `~/Applications/ScreenshotMacro.app`에 생성합니다:
