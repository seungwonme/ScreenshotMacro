...
def calculate_image_hash(
    image_path: Path, logger: logging.Logger
    ...
    try:
        ...
        logger.error(f"이미지 처리 중 오류 발생 {image_path}: {e}")
        return None
...
def find_duplicate_images(
    ...
    logger.info(f"검색 디렉토리: {directory}")
...
def display_duplicate_groups(
    ...
    logger.info(
        f"총 {total_groups}개 그룹에서 {total_files}개의 유사/중복 이미지를 발견했습니다."
    )

    for i, (hash_value, file_list) in enumerate(duplicates.items(), 1):
        logger.info(f"\n유사한 이미지 그룹 #{i} (해시: {hash_value}):")
