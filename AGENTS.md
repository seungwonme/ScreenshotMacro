...
- `Sources/SMacroCore/Core.swift`: CLI/GUI 공유 로직.
  ...
  - 팝업(모달) 정크 프레임: 기준 이미지 중앙 밴드(`popupCrop`, 가로 28~72% x 세로 42~58%)에 최빈 배경색과 다른 콘텐츠 픽셀이 충분하면 모달형으로 간주, 배경 페이지가 매번 달라도 밴드만 비교해 매칭 - "마지막 페이지입니다" 팝업 프레임용. 실측: 팝업 프레임 밴드 거리 0 vs 최근접 실제 페이지 39.7. 모달형 패턴이 없으면 밴드 계산 자체를 건너뜀
