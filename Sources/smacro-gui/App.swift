...
struct ContentView: View {
    ...
    private var stepSettings: some View {
        Form {
            ...
            Section("타이밍") {
                ...
                Toggle("끝나면 중복·정크 프레임(로딩 화면 등) 자동 정리", isOn: $dedupOnFinish)
                if dedupOnFinish {
    ...
    private func startMacro() {
        ...
        let task = Task {
            ...
            do {
                ...
                let dedupNote = removed > 0 ? " (중복·정크 프레임 \(removed)장 휴지통 이동, \(kept)장 유지)" : ""
                if Task.isCancelled {
    ...
    /// 등록된 정크 프레임(로딩 화면 등)과 거의 같은 프레임은 전부 이동.
    ...
    private func pruneDuplicates(in dir: URL) -> Int {
        ...
        // 정크 프레임 매칭은 파일이 남아 있을 때(삭제 전) 계산
        let junk = junkMatches(in: files, patterns: (try? junkPatterns()) ?? [])
        var removed = 0
